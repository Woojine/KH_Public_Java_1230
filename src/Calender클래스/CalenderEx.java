...
// Calender 클래스는 추상 클래스이므로 객체를 생성 할 수 없음, 정적메소드인 getInstance() 메소드로 운영체제의 시간을 가져 와야됨
// Calender 클래스 포함된 모든 필드는 정적 변수(클래스 변수)
public class CalenderEx {
    public static void main(String[] args) {
        Calendar now = Calendar.getInstance(); // Date now = new Date();
        System.out.println(now.get(Calendar.YEAR));
        System.out.println(now.get(Calendar.MONTH)+1); // 달의 정보는 배열의 인덱스로 넘어 오기 때문에 1을 더해야 한다
        System.out.println(now.get(Calendar.DAY_OF_MONTH)); // 달에서 몇번째 날짜
        System.out.println(now.get(Calendar.DAY_OF_WEEK)); // 주에서 몇번째 날까(일요일 부터 시작하면 1부터)
