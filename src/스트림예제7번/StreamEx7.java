...
// 최종 연산시 모든 요소를 소모한 해당 스트림은 더 이상 사용 불가
// 요소의 출력 : forEach()
// 요소의 소모 : reduce() => 첫번째와 두번째 요소를 가지고 연산을 수행한 뒤, 그 결과와 세번째 요소를 가지고 또 수행
// Optional : NPE(Null Pointer Exception)을 방지 할 수 있도록 도와주는 클래스
